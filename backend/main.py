...
def create_inquiry(request: InquiryRequest):
    ...
    if not question:
        raise HTTPException(status_code=422, detail="問い合わせ内容を入力してください。")
...
"""
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="AI の応答を JSON として解析できませんでした。再度お試しください。")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"AI 処理中にエラーが発生しました: {str(e)}")
